...
fn main() {
    ...
    let strings = vec!["Alfa", "Beta", "Gamma"];
    for s in &strings {
        let c_string = CString::new(*s).expect("CString::new failed");
        let raw_ptr = c_string.into_raw(); // rust gives up ownership, C must free it
        unsafe {
            print(raw_ptr); // C will print and free it
        }
    }
    ...
    // DOUBLE FREE
    let others_strings = vec!["Delta", "Epsilon", "Zeta"];
    for a in &others_strings {
