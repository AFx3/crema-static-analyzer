    ...
    where F: Fn(&Predicate, &Env) -> bool{    
        ...
        where F: Fn(&Predicate, &Env) -> bool {
            ...
            match stmt {
                ...
                // taint_src: alloc(*) |> taint_src: write(*)
                // There exists some temporal path (information flow) where alloc(*) happens before write(*) 
                /*  BASE, NOT TEMPORAL
                Statement::Then(lhs, rhs) => {
                    // "E (lhs U rhs)" semantics — temporal sequencing in the same abstract environment
                    let lhs_holds = rec(lhs, f, env_mut, _variables, possible_values, anon_counter);
                    let rhs_holds = rec(rhs, f, env_mut, _variables, possible_values, anon_counter);
                    lhs_holds && rhs_holds
                }*/
                Statement::Then(lhs, rhs) => {
            

                    // Collect the named variables to reassign when generating successors
                    let var_names: Vec<String> = _variables.iter()
                        .filter_map(|v| match &v.name {
                            Some(VarName::Named(n)) => Some(n.clone()),
                            _ => None,
                        }).collect();

                    // deterministic key for Env (for visited set)
                    let env_key = |e: &Env| {
                        let mut keys: Vec<_> = e.keys().cloned().collect();
                        keys.sort();
                        keys.into_iter()
                            .map(|k| format!("{}={}", k, e.get(&k).unwrap_or(&"<nil>".to_string())))
                            .collect::<Vec<_>>()
                            .join(";")
                    };

                    // REQUIRE: lhs must hold at the current env (s0). If not, no path from s0 qualifies
                    if !rec(lhs, f, env_mut, _variables, possible_values, anon_counter) {
                        return false;
                    }

                    // if rhs already holds at s0, we've satisfied the property (k = 0) AND lhs holds at s0
                    if rec(rhs, f, env_mut, _variables, possible_values, anon_counter) {
                        return true;
                    }

                    // if we cannot generate successors, cannot find a future rhs
                    let pv_len = possible_values.len();
                    if var_names.is_empty() || pv_len == 0 {
                        return false;
                    }

                    // BFS from s0 to find a state where rhs holds and lhs holds on the path prefixes.
                    let mut queue: VecDeque<Env> = VecDeque::new();
                    queue.push_back(env_mut.clone());
                    let mut visited: HashSet<String> = HashSet::new();

                    while let Some(state) = queue.pop_front() {
                        let key = env_key(&state);
                        if visited.contains(&key) { continue; }
                        visited.insert(key.clone());

                        // if rhs holds at this state, found a witness path (ensured lhs held on the path when enqueueing)
                        let mut st_for_rhs = state.clone();
                        if rec(rhs, f, &mut st_for_rhs, _variables, possible_values, anon_counter) {
                            return true;
                        }
                        // ff lhs fails here, cannot extend this path
                        let mut st_for_lhs = state.clone();
                        if !rec(lhs, f, &mut st_for_lhs, _variables, possible_values, anon_counter) {
                            continue;
                        }
                        // generate successors by assigning each named var to each possible value (cartesian product)
                        let m = var_names.len();
                        let mut indices = vec![0usize; m];
                        loop {
                            let mut succ = state.clone();
                            for (idx, var_name) in var_names.iter().enumerate() {
                                succ.insert(var_name.clone(), possible_values[indices[idx]].clone());
                            }

                            let succ_key = env_key(&succ);
                            if !visited.contains(&succ_key) {
