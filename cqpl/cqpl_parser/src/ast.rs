...
impl Statement {
    /// Evaluate this logical statement, given:
    /// - f: a predicate evaluator function that takes a Predicate and the current variable environment, and returns a boolean result
    /// - env: the current environment mapping variable names to concrete values
    /// - variables: the list of variables currently in scope
    ///
    /// This recursively evaluates the statement, substituting variables with concrete values and applying the logical operators
    pub fn eval_with_env<F>(&self, f: &F, env: &Env, variables: &[String]) -> bool
    where
        // The predicate evaluator f must be a function or closure taking a Predicate and Env, returning a bool
        F: Fn(&Predicate, &Env) -> bool,
    {
        // Pattern-match on the kind of Statement eval:
        match self {
            // if the statement is a Predicate, call the evaluator f with the predicate and environment
            Statement::Predicate(p) => f(p, env),

            // if it's a negation (!), recursively evaluate the inner statement and negate the result
            Statement::Not(inner) => !inner.eval_with_env(f, env, variables),

            // if it's an AND, recursively evaluate left and right sub-statements and AND their results
            Statement::And(lhs, rhs) => lhs.eval_with_env(f, env, variables) && rhs.eval_with_env(f, env, variables),

            // if it's an OR, recursively evaluate left and right sub-statements and OR their results
            Statement::Or(lhs, rhs) => lhs.eval_with_env(f, env, variables) || rhs.eval_with_env(f, env, variables),

            // if it's a wildcard (matches anything), always return true
            Statement::Wildcard => true,

            // if it's a THEN operator (often logical implication or sequencing), treat it as AND in this evaluation
            Statement::Then(lhs, rhs) => lhs.eval_with_env(f, env, variables) && rhs.eval_with_env(f, env, variables),

            // if the statement is quantified, i.e., involves a quantifier 
            Statement::Quantified { quant, var, cond } => {
                // extract the variable name from the quantifier variable;
                // it must be a named variable, otherwise panic (this enforces correct structure).
                let var_name = match var {
                    Some(VarName::Named(name)) => name,
                    _ => panic!("Quantified variable must have a named VarName"),
                };

                // handle the quantifier type (ForAll or Exists):
                match quant {
                    Quantifier::ForAll => {
                        // for forall, require that the condition holds for *all* possible values of the quantified variable.

                        // iterate over all current variables' values and check condition:
                        variables.iter().all(|val| {
                            // clone the current environment to avoid mutation side effects:
                            let mut env_new = env.clone();

                            // insert the quantified variable binding into the environment:
                            env_new.insert(var_name.clone(), val.clone());

                            // recursively evaluate the condition with the new environment:
                            cond.eval_with_env(f, &env_new, variables)
                        })
                    }
                    Quantifier::Exists => {
                        // for Exists, require that there is *at least one* value for which the condition holds true

                        // iterate over variable values, return true on first success:
                        variables.iter().any(|val| {
                            let mut env_new = env.clone();
                            env_new.insert(var_name.clone(), val.clone());
                            cond.eval_with_env(f, &env_new, variables)
                        })
