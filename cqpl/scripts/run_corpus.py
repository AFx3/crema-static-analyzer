#!/usr/bin/env python3
"""Self-contained CREMA/CQPL corpus runner for artifact evaluation.

This runner intentionally has NO dependency on legacy CREMA detector outputs and
NO comparison against historical CQPL results. It executes the selected CQPL
query set on every active corpus target and archives raw evidence.

Schema 2 query set:
  leak_alloc.cqpl, double_free_alloc.cqpl, use_after_free_alloc.cqpl,
  allocator_mismatch_ub.cqpl

Schema 1 compatibility query set:
  leak.cqpl, double_free.cqpl, use_after_free.cqpl
"""
...
QUERY_FILES = {
    ...
    2: {
        "leak": "leak_alloc.cqpl",
        "double_free": "double_free_alloc.cqpl",
        "use_after_free": "use_after_free_alloc.cqpl",
...
def timeout_run(cmd: list[str], *, cwd: Path | None, log: Path, timeout: float) -> tuple[int, bool]:
...
def main() -> int:
    ...
    query_files = QUERY_FILES[args.schema_version]
    ...
    for idx, (rel, target) in enumerate(selected, 1):
        ...
        rc, timed = timeout_run(build_cmd, cwd=target, log=tdir / "build.log", timeout=args.build_timeout)
        item["build_exit"] = rc
        ...
        if args.schema_version == 2:
            cmd += ["--allocation-identity-out", str(identity)]
    ...
    aggregate = {
        "protocol": f"CREMA-CQPL-corpus-schema-v{args.schema_version}-no-legacy-comparison",
