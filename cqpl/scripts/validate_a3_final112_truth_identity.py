...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description=(
            "Fail-closed A3 FINAL112 gate: preserve the historical audit and "
            "admit only reviewed oracle-consistent precision deltas"
        )
    )
    ...
    ap.add_argument(
        "--approved-deltas",
        type=Path,
        default=root / "artifact" / "A3_R4_FINAL112_PRECISION_DELTAS.tsv",
    )
    ap.add_argument(
        "--source-oracle-audit",
        type=Path,
    ...
    delta_rows = read_tsv(args.approved_deltas)
    ...
    for row in delta_rows:
        ...
        if (old, new) != ("unk", "ff"):
            allowlist_errors.append(f"A3 R4 only admits reviewed unk->ff refinements, got {key}: {old}->{new}")
    ...
    result = {
        "schema": "cqpl_a3_final112_precision_delta_audit_v1",
    ...
    if not passed:
        return 3
    print(
        "A3_FINAL112_PRECISION_AUDIT: PASS "
