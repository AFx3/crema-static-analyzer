...
fn is_raw_address_rvalue(e: &str) -> bool {
    let e = e.trim();
    e.starts_with("&raw const ") || e.starts_with("&raw mut ")
}
    ...
    match stmt.kind.as_str() {
        ...
        "Assign" => {
            if let Some(rvalue) = &stmt.rvalue {
                ...
                if is_raw_address_rvalue(rvalue) {
                    // AddressOf creates a raw pointer to a place.  Without place
                    // type/provenance we must not confuse a pointer to a stack
                    // local (e.g. a Box object) with the heap allocation managed
                    // by that local.
